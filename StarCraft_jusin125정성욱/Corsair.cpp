...
void CCorsair::Initialize(void)
{
	...
	// 유닛의 정보 - Probe로 설정
	Set_UnitInfo(UNIT_NAME::Corsair);
...
void CCorsair::Release(void)
{
}


void CCorsair::Attack(CObj * _pObj)
{
}

void CCorsair::Update_State(void)
{
	list<CObj*> lst;
	int fX, fY;
	switch (m_eUnitState)
	...
	case UNIT_STATE::Attack:
		...
		if (dwFrameTime + dwFrameSpeed < GetTickCount())
		{
			...
			POINT pt = m_pTarget->Get_Pos();

			dynamic_cast<CUnit*>(m_pTarget)->Set_Damage(m_tUnitInfo.iAttack);
			CEffectMgr::Get_Instance()->Create_Effect(EFFECT::CORSAIR_SPLASH, pt.x, pt.y, 60, 60);
		}
